...
async def chat_waifu(session):
    await session.send('まったく、あなたという人は本当に頑固ね……これじゃ私、もう全力であなたを守るしかないじゃない。')


@sv.on_command('老公', only_to_me=True)
async def chat_laogong(session):
    await session.send('またたきせずに自分の死に様を見届けることね!', at_sender=True)
...
async def ddhaole(session):
    await session.send('周りに誰もいない部屋をお願い...')


@sv.on_command('我好了', only_to_me=False)
async def nihaole(session):
    await session.send('ちょっと近いわ……')
